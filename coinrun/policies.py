...
def impala_cnn(images, depths=[16, 32, 32]):
    ...
    def dropout_layer(out):
        if Config.DROPOUT > 0:
            ...
            batch_seed = tf.get_variable(
                var_name,
                shape=batch_seed_shape,
                initializer=tf.random_uniform_initializer(minval=0, maxval=1),
                trainable=False,
            )
            batch_seed_assign = tf.assign(
                batch_seed, tf.random_uniform(batch_seed_shape, minval=0, maxval=1)
            )
...
class CnnPolicy(object):
    def __init__(
        self, sess, ob_space, ac_space, nbatch, nsteps, **conv_kwargs
