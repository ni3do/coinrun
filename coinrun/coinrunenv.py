...
import random
import sys
from ctypes import c_bool, c_char_p, c_float, c_int
...
def init_args_and_threads(cpu_count=4, monitor_csv_policy="all", rand_seed=None):
    ...
    lib.initialize_set_monitor_dir(
        logger.get_dir().encode("utf-8"), {"off": 0, "first_env": 1, "all": 2}[monitor_csv_policy]
    )
...
class CoinRunVecEnv(VecEnv):
    ...
    def __init__(self, game_type, num_envs, lump_n=0, default_zoom=5.0):
        ...
        self.handle = lib.vec_create(
            game_versions[game_type], self.num_envs, lump_n, self.hires_render, default_zoom
        )
